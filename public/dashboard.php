...
<div class="welcome-greeting">
  <h1><span class="greeting-emoji"><?= $greetEmoji ?></span> <?= h($greetText) ?>, <?= h($firstName) ?></h1>
  <p>Here's your job search overview for today.</p>
</div>

<!-- Stats -->
<div class="stat-grid">
  <div class="stat-card stat-card--blue">
    <div class="stat-icon stat-icon--blue"><i class="bi bi-briefcase"></i></div>
    <div class="stat-label">Total Applications</div>
    <div class="stat-value"><?= h($total) ?></div>
  </div>
  <div class="stat-card stat-card--yellow">
    <div class="stat-icon stat-icon--yellow"><i class="bi bi-chat-dots"></i></div>
    <div class="stat-label">Interviewing</div>
    <div class="stat-value"><?= h($counts['interviewing']) ?></div>
    <span class="stat-delta stat-delta--up"><i class="bi bi-arrow-up-short"></i> Active</span>
  </div>
  <div class="stat-card stat-card--green">
    <div class="stat-icon stat-icon--green"><i class="bi bi-trophy"></i></div>
...
<div class="row g-4">
        ...
        <div class="kanban-board">
          ...
          <div class="kanban-col">
            ...
            <div class="kanban-cards">
              ...
              <?php endforeach; ?>
              <?php if (empty($apps)): ?>
              <div class="text-center py-3" style="font-size:13px;color:var(--text-tertiary)">No applications</div>
              <?php endif; ?>
            </div>
          </div>
          <?php endforeach; ?>
  ...
  <div class="col-lg-4">
    <div class="card mb-4">
      ...
      <div class="card-body p-0">
        <?php if (empty($upcoming)): ?>
          <div class="text-center py-4" style="font-size:13px;color:var(--text-tertiary)">No upcoming events</div>
        <?php else: ?>
          ...
          <div class="d-flex gap-3 p-3 border-bottom">
            ...
            <div>
              ...
              <div style="font-size:12px;color:var(--text-tertiary)"><?= h($ev['company']) ?> &bull; <?= date('g:i A', strtotime($ev['event_date'])) ?></div>
            </div>
          </div>
          <?php endforeach; ?>
        <?php endif; ?>
    ...
    <!-- Stage breakdown -->
    <div class="card" style="background:var(--sidebar-bg);color:#fff;border-color:rgba(255,255,255,0.06);">
      <div class="card-body p-3">
        <div style="font-size:11px;opacity:.5;margin-bottom:4px;text-transform:uppercase;letter-spacing:0.5px;font-weight:600">Monthly Status</div>
        <div style="font-size:28px;font-weight:700;font-family:'Sora',sans-serif;margin-bottom:16px">
          <?= $total ?> <span style="font-size:14px;font-weight:400;opacity:.5">Active Apps</span>
        </div>
        <?php foreach (['interviewing'=>'Interviews Scheduled','offer'=>'Offers'] as $s => $label): ?>
        <div class="d-flex justify-content-between align-items-center mb-2">
          <span style="font-size:13px;opacity:.7"><?= $label ?></span>
          <span style="font-weight:700"><?= $counts[$s] ?></span>
        </div>
        <div class="progress progress-thin mb-3" style="background:rgba(255,255,255,.1)">
